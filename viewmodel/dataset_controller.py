...
class DatasetController(QObject):
    ...
    def cargar_datasets(self):
        ...
        if not self.DATASET_FILE.exists():
            with open(self.DATASET_FILE, "w", encoding="utf8") as archivo:
                json.dump([], archivo, indent=4, ensure_ascii=False)
        with open(self.DATASET_FILE, "r", encoding="utf8") as archivo:
    ...
    def generar_id(self):
        if len(self.datasets) == 0:
            return "DS-001"
        numeros = [int(ds["id"].split("-")[1]) for ds in self.datasets]
        return f"DS-{max(numeros)+1:03d}"

    @Slot(str)
    def eliminarDataset(self, id_dataset):
        self.datasets = [ds for ds in self.datasets if ds["id"] != id_dataset]
    ...
    def analizar_dataset(self, ruta):
        ...
        with open(ruta, "r", encoding="utf8") as archivo:
            for linea in archivo:
                ...
                texto = ""
                for campo in ["instruction", "context", "response"]:
                    if campo in objeto:
                        texto += objeto[campo] + " "
                texto = texto.strip()
                if texto == "":
                ...
                if longitud_minima is None:
                    ...
                    longitud_minima = min(longitud_minima, longitud)
